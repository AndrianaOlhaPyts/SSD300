...
class PredictionConvolutions(nn.Module):
    ...
    def forward(self, conv4_3_feats, conv7_feats, conv8_2_feats, conv9_2_feats, conv10_2_feats, conv11_2_feats):
        ...
        # Передбачити межі блоків локалізації 
        ...
        # Всього 8732 коробки
        # Конкатенація в цьому конкретному порядку 
...
class SSD300(nn.Module):
    ...
    def forward(self, image):
        # Виконайте згортки базової мережі VGG 
    ...
    # Декодує 8732 місця та рейтинги класів
...
# Функція втрати MultiBox для виявлення об’єктів.
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predicted_locs, predicted_scores, boxes, labels):
        ...
        for i in range(batch_size):
            ...
            # Потім призначте кожному об’єкту відповідний пріоритет максимального перекриття. 
            object_for_each_prior[prior_for_each_object] = torch.LongTensor(range(n_objects)).to(device)

            # Щоб переконатися, що ці пріоритети кваліфікуються, штучно дайте їм перекриття більше 0,5. 
            overlap_for_each_prior[prior_for_each_object] = 1.

            # Мітки для кожного пріора
            label_for_each_prior = labels[i][object_for_each_prior]  
            # Встановити пріоритети, перекриття яких з об’єктами менше порогового значення, щоб бути фоном 
