...
# Колірна карта для обмежувальних рамок виявлених об’єктів 
...
# Перетворіть обмежувальні прямокутники з граничних координат на координати центрального розміру 
def xy_to_cxcy(xy):
    return torch.cat([(xy[:, 2:] + xy[:, :2]) / 2,  
                      xy[:, 2:] - xy[:, :2]], 1)  

# Перетворіть обмежувальні рамки з координат центру на граничні координати 
def cxcy_to_xy(cxcy):
...
# Знайдіть перетин кожної комбінації коробок між двома наборами коробок
def find_intersection(set_1, set_2):
    lower_bounds = torch.max(set_1[:, :2].unsqueeze(1), set_2[:, :2].unsqueeze(0))  
    upper_bounds = torch.min(set_1[:, 2:].unsqueeze(1), set_2[:, 2:].unsqueeze(0))  
    intersection_dims = torch.clamp(upper_bounds - lower_bounds, min=0)  
    return intersection_dims[:, :, 0] * intersection_dims[:, :, 1]  
# Знайдіть перекриття Jaccard (IoU) кожної комбінації коробок між двома наборами коробок
